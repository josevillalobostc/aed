...
template <typename key_type, typename value_type> struct my_map {
  ...
  long long _hash_single(long long key) const {
    const int B = 311;
    const int MOD = 1e9 + 7;
  ...
  int _hash(const key_type &key) const {
    long long h1 = _hash_single(key.first);
    long long h2 = _hash_single(key.second);
    long long combined = h1 ^ (h2 + 0x9e3779b9 + (h1 << 6) + (h1 >> 2));
    return llabs(combined) % m;
  }
  void print() {
    for (int i = 0; i < m; ++i) {
      cout << "Bucket " << i << ": " << endl;
...
int main() {
  ...
  while (t--) {
    ...
    long long cant = 0;
    cin >> n >> x >> y;
    my_map<pair<int, int>, int> map(2 * n);
    while (n--) {
      int a;
      cin >> a;
      if (map.has_key({(x - (a % x)) % x, a % y})) {
        cant += map[{(x - (a % x)) % x, a % y}];
      }
      map[{a % x, a % y}] += 1;
    }
    cout << cant << '\n';
